...
async def simple_pdf_analysis():
    ...
    try:
        ...
        if result["success"]:
            ...
            print(f"💰 Cost: ${result['metadata'].get('total_cost_in_cents', 0) / 100:.4f}")
            print(f"📊 Tokens: {result['metadata'].get('total_input_tokens', 0)} input, {result['metadata'].get('total_output_tokens', 0)} output")
...
async def test_large_pdf():
    ...
    try:
        ...
        if result["success"]:
            ...
            print(f"📊 Total tokens: {metadata.get('total_input_tokens', 0)} input, {metadata.get('total_output_tokens', 0)} output")
            print(f"⚡ Cached tokens: {metadata.get('cached_tokens', 0)}")
            
            print("\n--- Analysis Result ---")
            print(result["result"][:2000] + "..." if len(str(result["result"])) > 2000 else result["result"])
        else:
...
async def test_caching_behavior():
    ...
    if result1["success"]:
        ...
        print(f"💰 Cost: ${result1['metadata'].get('total_cost_in_cents', 0) / 100:.4f}")
    ...
    if result2["success"]:
        ...
        print(f"💰 Cost: ${result2['metadata'].get('total_cost_in_cents', 0) / 100:.4f}")
        
        # Compare caching
        if result2['metadata'].get('cached_tokens', 0) > result1['metadata'].get('cached_tokens', 0):
