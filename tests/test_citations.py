...
class TestCitations(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_simple_anthropic_citations(self):
        ...
        response = await citations_tool(
            question, instructions, documents, "claude-3-7-sonnet-latest", LLMProvider.ANTHROPIC
        )
        ...
        for block in response:
            if block.get("type") == "text" and "pizza" in block.get("text", "").lower():
                found_pizza = True
                break
        self.assertTrue(found_pizza, "Response should mention pizza as Rishabh's favourite food")
    ...
    async def test_complex_anthropic_citations(self):
        ...
        response_text = " ".join([block.get("text", "") for block in response if block.get("type") == "text"]).lower()
    ...
    async def test_no_relevant_documents_anthropic(self):
        ...
        response_text = " ".join([block.get("text", "") for block in response if block.get("type") == "text"]).lower()
        self.assertTrue(
            "don't know" in response_text or "not" in response_text or "no information" in response_text,
    ...
    def test_unsupported_provider(self):
        ...
        with self.assertRaises(ValueError) as context:
            ...
            import asyncio
            asyncio.run(citations_tool(question, instructions, documents, "test-model", "unsupported_provider"))
