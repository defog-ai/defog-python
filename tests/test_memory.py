...
class TestMemoryManager:
    ...
    def test_get_messages_for_compactification(self):
        ...
        system_msgs, to_summarize, to_preserve = manager.get_messages_for_compactification()
...
class TestTokenCounter:
    ...
    def test_count_tokens_different_providers(self):
        ...
        anthropic_tokens = counter.count_tokens(messages, "claude-3-sonnet", "anthropic")
...
class TestCompactifyMessagesReal:
    ...
    async def test_compactify_messages_with_summary(self):
        ...
        to_summarize = [
            ...
            {"role": "assistant", "content": "I don't have access to real-time weather data, but Paris typically has mild weather."},
            {"role": "user", "content": "What about in summer?"},
            {"role": "assistant", "content": "Paris summers are usually warm, with temperatures around 20-25°C (68-77°F)."}
        ]
        preserved = [
            {"role": "user", "content": "Should I pack a jacket for July?"}
        ]
...
class TestChatAsyncWithMemoryReal:
    ...
    async def test_conversation_with_compactification(self):
        ...
        conversation = [
            ...
            {"role": "assistant", "content": "Python features include dynamic typing, garbage collection, and extensive libraries."},
            {"role": "user", "content": "How do I install it?"},
            {"role": "assistant", "content": "You can install Python from python.org or using package managers."},
        ]
        ...
        summary_found = False
        for msg in messages:
            if msg["role"] == "user" and "[Previous conversation summary]" in msg["content"]:
                summary_found = True
...
class TestLongConversationWithMemory:
    ...
    async def test_long_conversation_multiple_compactifications(self):
        ...
        system_message = {"role": "system", "content": "You are a helpful programming assistant."}
            ...
            if current_compactifications > compactification_count:
                ...
                for msg in current_messages:
                    if msg["role"] == "user" and "[Previous conversation summary]" in msg["content"]:
                        ...
                        relevant_words = ["todo", "react", "backend", "api", "database", "component", "frontend", "express", "deploy", "authentication"]
                        assert any(word in summary_content for word in relevant_words), f"Summary doesn't contain relevant keywords: {summary_content[:200]}"
                        break
                
                assert summary_found, f"Summary not found after compactification {current_compactifications}"
                compactification_count = current_compactifications
            
            previous_token_count = current_token_count
        
        # Final assertions
        assert manager.history.compactification_count >= 2  # Should have at least 2 compactifications
        ...
        summary_count = sum(1 for msg in final_messages if "[Previous conversation summary]" in msg.get("content", ""))
