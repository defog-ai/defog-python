...
def validate_image_content(image_data: Dict[str, Any]) -> None:
    ...
    if image_data.get("type") not in ["image", "image_url"]:
        raise ProviderError("validation", f"Invalid image type: {image_data.get('type')}")
    ...
    if image_data.get("type") == "image":
        ...
        if source.get("type") == "base64":
            ...
            if not source.get("media_type"):
                raise ProviderError("validation", "Media type is required for base64 images")
        elif source.get("type") == "url":
            if not source.get("url"):
                raise ProviderError("validation", "URL is required for URL images")
        else:
            raise ProviderError("validation", f"Invalid image source type: {source.get('type')}")
    ...
    try:
        with httpx.Client() as client:
            ...
            if not mime_type.startswith('image/'):
                raise ProviderError("download", f"URL returned non-image content type: {mime_type}")
        ...
        if block.get("type") == "text":
            ...
            if block.get("type") == "image":
                ...
                if source.get("type") == "base64":
                    ...
                    image_data, mime_type = download_image_from_url(source.get("url", ""))
            elif block.get("type") == "image_url":
                url = block.get("image_url", {}).get("url", "")
                if url.startswith("data:"):
