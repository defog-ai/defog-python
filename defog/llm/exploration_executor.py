...
class ExplorationExecutor:
    ...
    async def generate_exploration_paths(
        ...
        if self.shared_context:
            paths_key = f"exploration_paths/{task.agent_id}/{datetime.now().isoformat()}"
    ...
    async def _execute_exploration_path(
        ...
        try:
            ...
            if self.shared_context:
                result_key = f"exploration_result/{path.path_id}/{datetime.now().isoformat()}"
                await self.shared_context.write_artifact(
    ...
    async def _record_success(
        ...
        if self.shared_context:
            pattern_key = f"successful_pattern/{task_pattern}/{datetime.now().isoformat()}"
            await self.shared_context.write_artifact(
                agent_id=task.agent_id,
                key=pattern_key,
                content={
