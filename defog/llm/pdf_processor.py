...
class PDFAnalysisInput(BaseModel):
    ...
    response_format: Optional[Type[BaseModel]] = Field(
        default=None,
        description="Pydantic model for structured response"
    )
...
Provide a thorough analysis based on the PDF content."""
    ...
    def _combine_chunk_results(
        ...
        if len(successful_results) > 1:
            combined_content.append(f"PDF Analysis Summary ({len(successful_results)} chunks processed):")
    ...
    async def analyze_pdf(
        ...
        try:
            ...
            # Combine results
            if response_format and len(pdf_chunks) == 1 and chunk_results[0].get("success"):
            ...
            total_input_tokens = sum(r.get("input_tokens") or 0 for r in chunk_results if r.get("success"))
            total_output_tokens = sum(r.get("output_tokens") or 0 for r in chunk_results if r.get("success"))
            total_cost = sum(r.get("cost_in_cents") or 0 for r in chunk_results if r.get("success"))
            total_cached_tokens = sum(r.get("cached_tokens") or 0 for r in chunk_results if r.get("success"))
            ...
            processing_metadata = {
                **metadata,
                ...
                "failed_chunks": len([r for r in chunk_results if not r.get("success")])
            }
