...
Conversation to summarize:"""
    ...
    async def prepare_context_for_new_agent(
        ...
        if include_cross_agent and self.shared_context:
            cross_agent_entries = await self.get_cross_agent_context(
                max_entries=5
            )
                ...
                context["recent_results"] = [
                    {
                        ...
                        "content": str(artifact.content)[:200] + "..." 
                            if len(str(artifact.content)) > 200 else artifact.content
                    }
    ...
    async def _cleanup_old_shared_memories(self) -> None:
        ...
        if len(self._shared_memory_keys) > self.max_shared_memory_entries:
            ...
            keys_to_remove = sorted_keys[:len(sorted_keys) - self.max_shared_memory_entries]
    ...
    async def compact_with_enhanced_summary(self) -> None:
        ...
        system_msgs, to_summarize, to_preserve = self.get_messages_for_compactification()
