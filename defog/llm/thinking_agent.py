...
class ThinkingAgent(Agent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logger.info(f"ThinkingAgent {agent_id} initialized with thinking_mode={enable_thinking_mode}")
...
Be thorough and consider multiple perspectives. This is your space to think deeply."""
            ...
            {
                "role": "user",
                "content": f"Think about this: {prompt}"
            }
        ]
        
        if context:
            thinking_messages[0]["content"] += f"\n\nContext:\n{json.dumps(context, indent=2)}"
...
4. When this approach would be most suitable"""
        ...
        try:
            ...
            if self.shared_context:
                exploration_key = f"exploration/{self.agent_id}/{datetime.now().isoformat()}"
                await self.shared_context.write_artifact(
                    agent_id=self.agent_id,
                    key=exploration_key,
                    content={
...
5. Rate the solution quality (1-10) with justification"""
        ...
        reflection_result = await self.think(
            ...
            save_to_shared_context=save_reflection
        )
    ...
    async def collaborate_with_agent(
        ...
        """
        if not self.shared_context or not isinstance(self.memory_manager, EnhancedMemoryManager):
        ...
        other_artifacts = await self.shared_context.list_artifacts(
            ...
            since=datetime.now().replace(hour=0, minute=0, second=0)  # Today's artifacts
        )
        ...
        cross_memories = await self.memory_manager.get_cross_agent_context(
            other_agent_ids=[other_agent_id],
            max_entries=5
        )
