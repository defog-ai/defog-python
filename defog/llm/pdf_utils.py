...
class PDFProcessor:
    ...
    def get_pdf_metadata(self, pdf_content: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            doc.close()
            logger.info(f"PDF metadata: {metadata['page_count']} pages, {metadata['size_mb']:.2f} MB")
            return metadata
    ...
    def should_split_pdf(self, metadata: Dict[str, Any]) -> bool:
        ...
        """
        return (
            metadata['page_count'] > self.MAX_PAGES_PER_CHUNK or
            metadata['size_bytes'] > self.MAX_SIZE_BYTES
        )
    
    def split_pdf_by_pages(self, pdf_content: bytes, metadata: Dict[str, Any]) -> List[bytes]:
        """
        ...
        try:
            ...
            for start_page in range(0, total_pages, pages_per_chunk):
                ...
                logger.info(f"Created chunk {len(chunks)}: pages {start_page+1}-{end_page+1}, {len(chunk_bytes)} bytes")
